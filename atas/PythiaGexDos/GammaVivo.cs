...
namespace PythiaGexDos
{
    ...
    /// </summary>
    [DisplayName("PythiaGex 2.0 - Gamma Vivo")]
    ...
    public class GammaVivo : Indicator
    {
                ...
                if (c != null && c.Filas.Count > 0)
                {
                    ...
                    try
                    {
                        var dst = Path.Combine(
                            ...
                            "ATAS", "pythiagex2-cadena-usada-" + Raiz() + ".json");
        ...
        private void PrepararRender()
        {
            ...
            try
            {
                ...
                {
                    bool adoptada = false;
                    var raw = AppDomain.CurrentDomain.GetData("PythiaGexDos.Prob." + inst) as string;
                    ...
                    if (!adoptada)
                        AppDomain.CurrentDomain.SetData("PythiaGexDos.Prob." + inst, string.Format(CultureInfo.InvariantCulture,
        ...
        private void LlenarYsNiveles(IChartContainer cont)
        {
            ...
            {
                string inst = (InstrumentInfo?.Instrument ?? "").ToUpperInvariant().TrimStart('#');
                if (inst.Length > 0) AppDomain.CurrentDomain.SetData("PythiaGexDos.Niveles." + inst, sb.ToString());
            }
            catch { }
        }
        ...
        private void VolcarCadenaViva()
        {
            ...
            try
            {
                ...
                File.WriteAllText(Path.Combine(
                    Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                    "ATAS", "pythiagex2-cadena-viva-" + Raiz() + ".json"), sb.ToString());
