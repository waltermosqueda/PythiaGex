...
namespace PythiaGexDos
{
    ...
    /// </summary>
    [DisplayName("PythiaGex 2.0 - Gamma Hoy")]
    ...
    public partial class GammaHoy : Indicator
    {
        ...
        [Display(Name = "Viva: profundidad, cuantos contratos al dinero (nivel 2)", GroupName = "1. Datos", Order = 10,
                 Description = "El nivel 2 de opciones se pide solo a los N contratos mas cercanos al precio del vencimiento mas proximo (24 = 12 strikes x call y put). Con la ventana entera (320) ATAS mostraba 'Market Data Latency' de 18-22 s y Rithmic cortaba la conexion de datos cada minuto (medido 16-09 y 17-09).")]
        [Range(2, 80)]
        public int ProfundidadContratos { get; set; } = 24;
        ...
        private void ArrancarViva()
        {
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    _viva.Profundidad = ProfundidadOpciones; _viva.ProfundidadContratos = Math.Max(2, ProfundidadContratos);
        ...
        private string RutaBaseRueda() => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "ATAS", "PythiaGex2", "base-rueda-" + Raiz() + ".json");
        ...
        private void RegistrarGatillos(IEnumerable<GatilloBanda.Marca> tiros, string fuente, bool pisar)
        {
            try
            {
                ...
                var ruta = Path.Combine(dir, "pythiagex2-gatillos-" + (fuente == "vivo" ? "" : "archivo-") + Lim(instr) + "-" + Lim(marco) + ".jsonl");
                var iv = CultureInfo.InvariantCulture;
        ...
        private static void Log(string msg)
        {
            try
            {
                var p = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "ATAS", "pythiagex2-gammahoy.log");
