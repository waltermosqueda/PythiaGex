 ...
 *   izquierda  velas (1/2/5/15 min), VWAP, los guiones amarillos de las dominantes por vela, los
 *              puntitos del zero por vela, las rayas (zero, majors, dominantes, pesadas), las bandas
 *              y los disparos (R = rebote, M = modelo, tren)
 *   derecha    el perfil por strike (GEX por volumen, con el OI detras), las pelotitas del Max
 *              Change (punta de cada barra hace 15/5/1 min) y la escalera de convexidad
 *   abajo      CVD (delta acumulado) y delta por vela, cuando hay order flow (solo desde ATAS)
 * Sin dependencias. El precio se dibuja en puntos del FUTURO (como en ATAS); el modo indice
 * resta la base.
 */
(function (global) {
  "use strict";
  const COL = {
    fondo: "#0b0f14", panel: "#0e141b", grilla: "#1a222c", texto: "#c9d3dd", tenue: "#5f6d7b", eje: "#8a97a6",
    alcista: "#3fbf7f", bajista: "#e5484d", mecha: "#8a97a6", dom: "#f2c14e", dom2: "#c9a03a", zero: "#a9b4c0", pos: "#3fbf7f", neg: "#e5484d",
    conv: "#9b7bff", conv2: "#5e4a9e", vwap: "#2fb6d6", banda: "rgba(242,193,78,0.10)", pel: "#dfe6ee", cero: "#ff8c42", cvd: "#3fbf7f", cvdNeg: "#e5484d",
    crosshair: "rgba(201,211,221,0.35)", pesada: "rgba(201,211,221,0.55)", marcador: "#dfe6ee",
  };
  ...
  class Grafico {
    constructor(canvas, opciones) {
      ...
      this.op = Object.assign({ modoIndice: false, base: 0, decimales: 2, verPerfil: true, verConv: true, verCvd: true, verGuiones: true, verZeroPorVela: true, verBandas: true,
                                bandaPct: 0.08, verMarcas: true, verVwap: true, verPelotitas: true, verOi: true, alturaCvd: 0.18, anchoPerfil: 0.30, velasVisibles: 180, tipoPerfil: "vol" }, opciones || {});
      this.datos = { velas: [], perfil: [], niveles: {}, marcas: [], cabecera: [], vwap: [], futuro: null, ultimaEdad: null };
      this.vista = { fin: null, n: this.op.velasVisibles, precioMin: null, precioMax: null, fijo: false };
      this.mouse = null;
      this._eventos();
    ...
    _eventos() {
      const cv = this.cv;
      cv.addEventListener("mousemove", e => { const r = cv.getBoundingClientRect(); this.mouse = { x: e.clientX - r.left, y: e.clientY - r.top }; this.render(); });
      cv.addEventListener("mouseleave", () => { this.mouse = null; this.render(); });
      cv.addEventListener("wheel", e => {
        e.preventDefault();
        const dir = Math.sign(e.deltaY);
        if (e.shiftKey) { this.vista.fin = Math.max(10, Math.min(this.datos.velas.length - 1, (this.vista.fin ?? this.datos.velas.length - 1) + dir * 5)); this.vista.fijo = this.vista.fin < this.datos.velas.length - 1; }
        else { this.vista.n = Math.max(30, Math.min(1500, Math.round(this.vista.n * (dir > 0 ? 1.15 : 0.87)))); }
        this.render();
      }, { passive: false });
      let arrastre = null;
      cv.addEventListener("mousedown", e => { arrastre = { x: e.clientX, fin: this.vista.fin ?? this.datos.velas.length - 1 }; });
      window.addEventListener("mouseup", () => { arrastre = null; });
      window.addEventListener("mousemove", e => {
        if (!arrastre) return;
        const dx = e.clientX - arrastre.x; const porVela = this._anchoVela || 6;
        const nuevo = Math.round(arrastre.fin - dx / porVela);
        this.vista.fin = Math.max(10, Math.min(this.datos.velas.length - 1, nuevo)); this.vista.fijo = this.vista.fin < this.datos.velas.length - 1; this.render();
      });
      cv.addEventListener("dblclick", () => { this.vista.fijo = false; this.vista.fin = this.datos.velas.length - 1; this.vista.n = this.op.velasVisibles; this.render(); });
      // tactil: arrastrar y pellizcar
      let toque = null;
      cv.addEventListener("touchstart", e => { if (e.touches.length === 1) toque = { x: e.touches[0].clientX, fin: this.vista.fin ?? this.datos.velas.length - 1 }; else if (e.touches.length === 2) toque = { d: Math.abs(e.touches[0].clientX - e.touches[1].clientX), n: this.vista.n }; }, { passive: true });
      cv.addEventListener("touchmove", e => {
        if (!toque) return;
        if (e.touches.length === 1 && toque.x != null) { const dx = e.touches[0].clientX - toque.x; this.vista.fin = Math.max(10, Math.min(this.datos.velas.length - 1, Math.round(toque.fin - dx / (this._anchoVela || 6)))); this.vista.fijo = this.vista.fin < this.datos.velas.length - 1; }
        else if (e.touches.length === 2 && toque.d) { const d = Math.abs(e.touches[0].clientX - e.touches[1].clientX); this.vista.n = Math.max(30, Math.min(1500, Math.round(toque.n * toque.d / Math.max(1, d)))); }
        this.render();
      }, { passive: true });
      cv.addEventListener("touchend", () => { toque = null; });
    }

    precio(v) { return this.op.modoIndice ? v - this.op.base : v; }   // de futuro a lo que se dibuja

    _layout() {
      const W = this.cv.clientWidth, H = this.cv.clientHeight;
      const ejeW = 66, ejeH = 22, cab = this.datos.cabecera.length ? 14 * this.datos.cabecera.length + 8 : 0;
    ...
    render() {
      ...
      // escala de precio: velas visibles + niveles cercanos
    ...
    _grilla(L, yDe) {
      const ctx = this.ctx; ctx.strokeStyle = COL.grilla; ctx.lineWidth = 1;
      const paso = this._pasoPrecio();
      for (let p = Math.ceil(this.pmin / paso) * paso; p <= this.pmax; p += paso) { const y = Math.round(L.velas.y1 - (p - this.pmin) / (this.pmax - this.pmin) * (L.velas.y1 - L.velas.y0)) + 0.5; ctx.beginPath(); ctx.moveTo(L.velas.x0, y); ctx.lineTo(L.perfil.x1, y); ctx.stroke(); }
    }
    _pasoPrecio() {
      const r = this.pmax - this.pmin; const objetivo = r / 9; const base = Math.pow(10, Math.floor(Math.log10(objetivo)));
    ...
    _bandas(L, yDe) {
      ...
      for (const d of n.doms) {
        const b = f * this.op.bandaPct / 100;
        const y1 = yDe(d.fut + b), y2 = yDe(d.fut - b);
        ctx.fillStyle = COL.banda; ctx.fillRect(L.velas.x0, y1, L.velas.x1 - L.velas.x0, y2 - y1);
      }
    }

    _raya(y, color, ancho, guion, x0, x1) {
      const ctx = this.ctx; ctx.save(); ctx.strokeStyle = color; ctx.lineWidth = ancho; ctx.setLineDash(guion || []); ctx.beginPath(); ctx.moveTo(x0, Math.round(y) + 0.5); ctx.lineTo(x1, Math.round(y) + 0.5); ctx.stroke(); ctx.restore();
    }
    _rayas(L, yDe) {
    ...
    _guiones(vis, xDe, yDe, w) {
      const ctx = this.ctx; const g = Math.max(2, Math.min(4, Math.round(w * 0.5)));
      const largo = Math.max(3, w * 0.9);
      for (let i = 0; i < vis.length; i++) {
        const v = vis[i], n = v.niv; if (!n) continue;
        const x = xDe(this.ini + i);
        [n.dom0, n.dom1].forEach((d, k) => {
          if (d == null) return;
          const y = yDe(d); if (y < this.L.velas.y0 || y > this.L.velas.y1) return;
          ctx.fillStyle = k === 0 ? COL.dom : COL.dom2; ctx.fillRect(x - largo / 2, y - (k === 0 ? g : g - 1) / 2, largo, k === 0 ? g : Math.max(1, g - 1));
        });
    ...
    _velas(vis, xDe, yDe, w) {
      const ctx = this.ctx; const cuerpo = Math.max(1, Math.floor(w * 0.62));
      for (let i = 0; i < vis.length; i++) {
    ...
    _marcas(vis, xDe, yDe, w, ini) {
      ...
      const ctx = this.ctx;
      const porT = new Map(); vis.forEach((v, i) => porT.set(v.t, ini + i));
      const dur = vis.length > 1 ? (vis[1].t - vis[0].t) : 60;
      for (const m of ms) {
        // la marca lleva la hora de CIERRE de la vela (como el indicador): la vela es la que abre dur antes
        ...
        if ((m.tipo || "").startsWith("rebote")) {
          const y = yDe(m.dom || m.precio), r = 4;
          ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.stroke();
          const rot = m.dz <= 1 ? "R1" : "R"; ctx.fillText(rot, x, m.lado > 0 ? y + r + 10 : y - r - 3);
        } else if ((m.tipo || "").startsWith("modelo")) {
          const y = yDe(m.lado > 0 ? v.l : v.h) + (m.lado > 0 ? 12 : -12), r = 5;
          ctx.beginPath(); ctx.moveTo(x, y - r); ctx.lineTo(x + r, y); ctx.lineTo(x, y + r); ctx.lineTo(x - r, y); ctx.closePath(); ctx.fill();
          ctx.fillText("M " + (m.dz != null ? m.dz.toFixed(2) : ""), x, m.lado > 0 ? y + r + 10 : y - r - 3);
        } else {
          const y = yDe(m.lado > 0 ? v.l : v.h) + (m.lado > 0 ? 10 : -10), r = 4;
          ctx.fillStyle = COL.dom; ctx.beginPath();
          if (m.lado > 0) { ctx.moveTo(x - r, y + r); ctx.lineTo(x + r, y + r); ctx.lineTo(x, y - r); } else { ctx.moveTo(x - r, y - r); ctx.lineTo(x + r, y - r); ctx.lineTo(x, y + r); }
          ctx.closePath(); ctx.fill(); ctx.fillText("tren", x, m.lado > 0 ? y + r + 10 : y - r - 3);
        }
        ctx.restore();
      }
    }
    ...
    _perfil(L, yDe) {
      ...
      // paso entre strikes en pixeles
      let alto = 6; if (vis.length > 1) { const ys = vis.map(s => yDe(s.fut)).sort((a, b) => a - b); let d = Infinity; for (let i = 1; i < ys.length; i++) d = Math.min(d, ys[i] - ys[i - 1]); alto = Math.max(2, Math.min(14, d * 0.72)); }
      const xc = x0 + gexW * 0.5;   // el eje cero del GEX en el medio del panel: barras positivas a la derecha, negativas a la izquierda
      ...
      for (const s of vis) {
        const y = yDe(s.fut), g = s[clave];
        const largo = Math.abs(g) / maxG * (gexW * 0.48);
        ...
        if (this.op.verPelotitas && s.antes) {
          s.antes.forEach((a, k) => { if (a == null) return; const la = Math.abs(a) / maxG * (gexW * 0.48); const xa = a >= 0 ? xc + la : xc - la; const r = [1.6, 2.4, 3.2][2 - k];
            ctx.fillStyle = "rgba(223,230,238,0.85)"; ctx.beginPath(); ctx.arc(xa, y, r, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = g >= 0 ? COL.pos : COL.neg; ctx.lineWidth = 0.8; ctx.stroke(); });
        }
        ...
        if (convW && s.conv != null) {
          const cx = x1 - convW + convW * 0.5, lc = Math.abs(s.conv) / maxC * (convW * 0.45);
          ctx.fillStyle = s.conv >= 0 ? COL.conv : COL.conv2; ctx.fillRect(s.conv >= 0 ? cx : cx - lc, y - alto / 2, lc, alto);
        }
      }
      if (convW) { ctx.fillStyle = COL.tenue; ctx.textAlign = "center"; ctx.textBaseline = "top"; ctx.fillText("convexidad", x1 - convW / 2, L.perfil.y0 + 2); ctx.fillText(clave === "gexVol" ? "GEX vol (OI detras)" : "GEX OI", x0 + gexW / 2, L.perfil.y0 + 2); }
      ctx.restore();
    }
    ...
    _ejePrecio(L, yDe) {
      ...
      const paso = this._pasoPrecio();
      for (let p = Math.ceil(this.pmin / paso) * paso; p <= this.pmax; p += paso) { const y = L.velas.y1 - (p - this.pmin) / (this.pmax - this.pmin) * (L.velas.y1 - L.velas.y0); ctx.fillText(fmtP(p, this.op.decimales), L.ejeX + 4, y); }
    ...
    _ejeTiempo(L, vis, xDe, w) {
      ...
      const cada = Math.max(1, Math.ceil(70 / Math.max(1, w)));
      const tz = this.op.zona || "America/Argentina/Buenos_Aires";
      for (let i = 0; i < vis.length; i += cada) {
        const t = new Date(vis[i].t * 1000); const txt = t.toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit", hour12: false, timeZone: tz });
        ctx.fillText(txt, xDe(this.ini + i), L.velas.y1 + L.cvdH + 5);
      }
    }
    ...
    _cvd(L, vis, xDe, w, ini) {
      ...
      // CVD desde la primera vela del dia de la rueda visible (se acumula sobre TODAS las velas cargadas)
      ...
      const yC = v => y1 - 4 - (v - mn) / (mx - mn) * (y1 - y0 - 8);
      const ym = (y0 + y1) / 2;
      ...
      ctx.stroke();
      ctx.font = "10px Consolas, monospace"; ctx.fillStyle = COL.tenue; ctx.textAlign = "left"; ctx.textBaseline = "top"; ctx.fillText("CVD " + Math.round(cvd[this.fin] || 0).toLocaleString("es-AR") + "  |  delta por vela", L.cvd.x0 + 6, y0 + 3);
    }
    ...
    _crosshair(L, vis, xDe, yDe, w, ini) {
      ...
      const i = Math.max(0, Math.min(vis.length - 1, Math.floor((m.x - L.velas.x0) / w)));
      const v = vis[i]; if (!v) return;
      const x = xDe(ini + i);
      ...
      const tz = this.op.zona || "America/Argentina/Buenos_Aires";
      const hora = new Date(v.t * 1000).toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit", hour12: false, timeZone: tz });
      ...
      const ancho = ctx.measureText(txt).width + 12; const bx = Math.min(m.x + 12, L.ejeX - ancho), by = Math.max(L.cab + 2, m.y - 22);
